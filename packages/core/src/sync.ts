...
function getBootstrapStatePath(): string {
  return resolve(getDSCodeHome(), ".bootstrap-state.json");
}
...
function removeEmptyParentDirectories(filePath: string, stopAt: string): void {
...
function syncManagedFiles(
  ...
  const sourcePaths = new Set(
    listFiles(sourceRoot).map((sourcePath) => relative(sourceRoot, sourcePath)),
  );

  // Remove files that no longer exist in source (if user hasn't edited them).
  ...
  // Copy or update files from source.
...
/**
 * Sync bundled assets (prompts, skills) from the app root to the DSCode home
 * directory using SHA-256 hash tracking. User-edited files are never overwritten.
 */
