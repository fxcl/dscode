...
function buildSectionList(section?: string, sections?: string[]): string[] {
  const tokens = [section, ...(sections ?? [])].filter(
...
function isLikelySectionHeading(line: string): boolean {
  return /^\s*(?:#{1,6}\s*)?(?:[0-9]+(?:\.[0-9]+)*\s+)?[A-Za-z][A-Za-z\s\-/]{2,80}:?\s*$/.test(
    line,
  );
}
...
export function extractPaperSections(
  ...
  const selected =
    typeof content === "string"
      ? pickTextSections(content, requested)
      : pickObjectSections(content, requested);
