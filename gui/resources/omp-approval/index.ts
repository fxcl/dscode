/**
 * dscode-approval — per-tool approval extension for the dscode/pi runtime,
 * bundled with DSCode GUI.
 *
 * Loaded into every GUI-spawned session via `dscode --mode rpc -e <this file>`.
 * Behavior is driven by a JSON config file whose path is passed in the
 * DSCODE_APPROVAL_CONFIG env var (written by the GUI on each session start):
 *
 *   { "mode": "off" | "writes" | "all", "locale": "zh" | "en" }
 *
 * - off:    every tool call is allowed (extension is inert)
 * - writes: shell/patch/file-write tools need approval, everything else passes
 ...
 *
 * Plain TypeScript, no dependencies beyond node:fs — the runtime loads this
 * with jiti.
 */
...
/** DSCode harness tools that execute shell commands or mutate the workspace. */
...
function loadConfig(): ApprovalConfig {
  const file = process.env.DSCODE_APPROVAL_CONFIG
...
function summarize(toolName: string, input: Record<string, unknown>): string {
  ...
  if (typeof input.input === 'string' && toolName === 'apply_patch') {
    const files = [...input.input.matchAll(/\*\*\* (?:Add|Update|Delete) File: (.+)/g)]
      .map((m) => m[1])
      .join(', ')
    return truncate(files || input.input, 160)
  }
...
export default function dscodeApproval(api: ExtensionApiLike): void {
