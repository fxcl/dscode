...
function runPi(args: string[]): Promise<PackageActionResult> {
  const cli = detectCli()
  if (!cli.available) {
    return Promise.resolve({ ok: false, log: 'dscode/omp/pi CLI not found' })
  }
...
export function installPackage(source: string): Promise<PackageActionResult> {
  return runPi(packageArgs(['install', source]))
}
...
export function updatePackage(source: string): Promise<PackageActionResult> {
  return runPi(packageArgs(['update', canonicalSourceForCommand(source, defaultPiAgentDir())]))
}
