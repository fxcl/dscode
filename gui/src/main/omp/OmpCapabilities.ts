...
/**
 * Runtime preference order: the DSCode CLI is the primary target; omp/pi are
 * kept as compatible fallbacks because DSCode speaks the same pi RPC lineage.
 * DSCODE_BIN overrides detection entirely (e.g. a checkout's dist/cli.js).
 */
const CLI_COMMANDS = ['dscode', 'omp', 'pi'] as const

export function detectCli(): CliInfo {
  if (cliInfoCache) return cliInfoCache

  const override = process.env.DSCODE_BIN
  if (override && existsSync(override)) {
    cliInfoCache = { command: 'dscode', path: override, available: true }
    return cliInfoCache
  }
  ...
  return { command: 'dscode', available: false }
}
