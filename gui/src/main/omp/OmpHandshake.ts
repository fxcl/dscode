...
export class OmpHandshake {
  ...
  private bootstrap(
    ...
    if (!ready) {
      ...
      this.state = 'failed'
      this.failure = { message: 'The runtime sent a malformed RPC ready frame.' }
    ...
    if (common.length === 0) {
      ...
      this.failure = {
        message:
          'This version of the runtime uses an RPC protocol that this version of DSCode GUI does not support.',
