...
/**
 * Read/write helpers for the runtime's own configuration files.
 * DSCode keeps everything under ~/.dscode (its PI_CODING_AGENT_DIR):
 * - ~/.dscode/settings.json — model defaults, project trust, packages
 * - ~/.dscode/auth.json     — provider credentials (api_key entries; OAuth
 *   blobs written by `dscode login` are preserved untouched)
 */

export function defaultPiAgentDir(cliCommand: string = detectCli().command): string {
  if (cliCommand === 'dscode') {
    // DSCode relocates the whole agent dir via DSCODE_HOME; settings/auth live
    // directly inside it (no nested "agent" segment).
    return process.env.DSCODE_HOME
      ? path.resolve(process.env.DSCODE_HOME)
      : path.join(homedir(), '.dscode')
  }
