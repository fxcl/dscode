...
import { installDSCode } from './installer'
...
let loginFlow: OmpLoginFlow | DscodeLoginFlow | null = null
...
export function registerIpc() {
  ...
  ipcMain.handle(IPC_CHANNELS.AUTH_START_LOGIN, async (_event, providerId: unknown) => {
    ...
    const cli = detectCli()
    // DSCode runs the SAME login code path in-process via @thinkany/dscode-core
    // (the CLI's `dscode login` demands a TTY the GUI cannot provide); every
    // other runtime rides the RPC `login` probe.
    if (cli.command === 'dscode') {
      const flow = new DscodeLoginFlow(broadcastLoginState)
      loginFlow = flow
  ...
  ipcMain.handle(
    ...
    async (_event, providerId: unknown, key: unknown) => {
      ...
      const cli = detectCli()
      // DSCode: API keys go straight into the runtime's own auth.json (the
      // file-backed path); no RPC probe exists for this profile.
      if (cli.command === 'dscode') {
        const result = setApiKey(providerId, key.trim())
        if (result.ok) {
          invalidateModelCache()
          runtimeSettings.invalidate()
        }
        return result.ok
          ? { ok: true }
          : { ok: false, error: result.log || 'failed to store the API key' }
      }
      const flow = new OmpLoginFlow({
        cli,
        onState: broadcastLoginState,
  ...
  ipcMain.handle(IPC_CHANNELS.OMP_INSTALL, async (event: IpcMainInvokeEvent) => {
    ...
    const success = await installDSCode((status: InstallStatus) => {
