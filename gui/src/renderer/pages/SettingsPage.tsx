...
export default function SettingsPage() {
  ...
  const [harness, setHarnessState] = useState<'minimal' | 'safe'>('minimal')
  const [sandbox, setSandboxState] = useState<'read-only' | 'workspace-write' | 'danger-full-access'>(
    'workspace-write'
  )
  ...
  const changeHarness = async (value: 'minimal' | 'safe') => {
    setHarnessState(value)
    await window.electronAPI.setStore('dscodeHarness', value)
  }

  const changeSandbox = async (
    value: 'read-only' | 'workspace-write' | 'danger-full-access'
  ) => {
    setSandboxState(value)
    await window.electronAPI.setStore('dscodeSandbox', value)
  }
      ...
      <div className="flex-1 overflow-y-auto p-5">
        <div className="mx-auto max-w-[680px] space-y-4">
          ...
          <Section title="DSCode CLI">
            ...
            <Row label={t('settings.harness')}>
              <div className="flex items-center gap-1">
                <button onClick={() => changeHarness('minimal')} className={seg(harness === 'minimal')}>
                  {t('settings.harnessMinimal')}
                </button>
                <button onClick={() => changeHarness('safe')} className={seg(harness === 'safe')}>
                  {t('settings.harnessSafe')}
                </button>
              </div>
            </Row>
            <Row label={t('settings.sandbox')}>
              <div className="flex items-center gap-1">
                <button
                  onClick={() => changeSandbox('read-only')}
                  className={seg(sandbox === 'read-only')}
                >
                  {t('settings.sandboxReadOnly')}
                </button>
                <button
                  onClick={() => changeSandbox('workspace-write')}
                  className={seg(sandbox === 'workspace-write')}
                >
                  {t('settings.sandboxWorkspaceWrite')}
                </button>
                <button
                  onClick={() => changeSandbox('danger-full-access')}
                  className={seg(sandbox === 'danger-full-access')}
                >
                  {t('settings.sandboxDanger')}
                </button>
              </div>
            </Row>
            <Row label={t('settings.webSearch')}>
              <div className="flex items-center gap-1">
                <button onClick={() => changeWebSearch(true)} className={seg(webSearch)}>
                  {t('settings.on')}
                </button>
                <button onClick={() => changeWebSearch(false)} className={seg(!webSearch)}>
                  {t('settings.off')}
                </button>
              </div>
            </Row>
