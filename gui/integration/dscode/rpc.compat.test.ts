 ...
 * actual dscode CLI (primary, legacy v1 profile), plus the installed omp
 * (current profile) and pi (legacy profile) when present.
 *
 * ISOLATION + CREDENTIAL-FREE: every spawn runs against a fresh temp
 * agent dir via `DSCODE_HOME`/`PI_CODING_AGENT_DIR` and a temp HOME;
 * provider credentials are stripped from the environment. No live model
 * inference happens (no "PONG"), so this suite consumes ZERO tokens and
 * never touches the developer's real config/auth. The zero-auth bootstrap
 * placeholder is used only so RPC mode boots (its provider is masked as
 * not-authenticated).
 *
 * Covered against the real wire:
 * - dscode: no ready frame → legacy v1 detection + get_state answers
 * - ready frame → negotiate_protocol → v2 activation (current omp)
 * - no ready frame → legacy v1 detection (legacy pi)
 ...
 *
 * Binaries: DSCODE_BIN / OMP_BIN / PI_BIN env overrides, else the repo's
 * dist/cli.js / `omp` / `pi`. A suite skips (does not fail) when its binary
 * is absent or the runtime cannot start.
 */
...
const BOOTSTRAP_KEY = 'dscode-gui-bootstrap-placeholder'

describe('DSCode CLI — legacy RPC v1 profile', () => {
  let available = false
  beforeAll(() => {
    available = binaryAvailable(DSCODE_BIN)
    if (!available) {
      console.warn(
        "[test:dscode] dscode binary not found — build the repo (pnpm build) or set DSCODE_BIN; skipping suite"
      )
    } else {
      console.log(`[test:dscode] '${DSCODE_BIN}' found — running dscode suite`)
    }
  })

  it('boots RPC mode: legacy v1 handshake, then get_state answers (isolated)', async () => {
    if (!available) return
    const live = startSession(DSCODE_BIN)
    try {
      await waitFor(() => live.session.handshakeOutcome !== null, 20_000, 'handshake')
      if (diedEarly(live)) {
        console.warn(`[test:dscode] runtime exited before handshake — stderr: ${live.stderrTail()}`)
        throw new Error(`dscode RPC did not start: ${live.stderrTail().slice(-500)}`)
      }
      const outcome = live.session.handshakeOutcome!
      expect(outcome.profile).toBe('legacy')
  ...
  it('permission flags: native --permission ask spawns cleanly', async () => {
    if (!available) return
    const live = startSession(DSCODE_BIN, ['--permission', 'ask'])
    try {
      await waitFor(() => live.session.handshakeOutcome !== null, 20_000, 'handshake')
      if (diedEarly(live)) {
        throw new Error(`dscode RPC did not start: ${live.stderrTail().slice(-500)}`)
  ...
  it('permission flags: --tools allowlist (readonly) spawns cleanly', async () => {
    ...
    const live = startSession(DSCODE_BIN, [
      '--tools',
      'update_plan,read_file,list_files,search_files,web_search'
    ])
    try {
      await waitFor(() => live.session.handshakeOutcome !== null, 20_000, 'handshake')
      if (diedEarly(live)) {
