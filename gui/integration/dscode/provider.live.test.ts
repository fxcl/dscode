 ...
 *
 * Run explicitly with:
 *   DSCODE_GUI_RUN_LIVE_TESTS=1 pnpm test:dscode:live
 *
 * May use configured provider credentials and consume tokens. It is never
 * invoked by `pnpm test`, `pnpm test:dscode`, or CI. Every mutation runs in an
 * isolated temp config/profile so the user's real default model, thinking,
 * skills, enabledModels and modelRoles are untouched.
 */
const RUN_LIVE =
  process.env.DSCODE_GUI_RUN_LIVE_TESTS === '1' || process.env.OMP_GUI_RUN_LIVE_TESTS === '1'

describe('live provider smoke test', () => {
  beforeAll(() => {
    if (!RUN_LIVE) console.warn('[test:dscode:live] skipped — set DSCODE_GUI_RUN_LIVE_TESTS=1 (may consume tokens)')
  })

  it('performs a single real inference against the runtime (opt-in)', async () => {
    if (!RUN_LIVE) return
    const bin =
      process.env.DSCODE_BIN ||
      (await import('node:path')).resolve(__dirname, '..', '..', '..', 'dist', 'cli.js')
    if (!binaryAvailable(bin)) {
      console.warn(`[test:dscode:live] '${bin}' not found — skipping`)
      return
    }
